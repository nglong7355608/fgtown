!(function($) {
    ...
    script.ajax = {
        ...
        callbacks: {
            account_donate: function ($form, data) {
                if (data.status === 'ok') {
            ...
            account_register: function ($form, data) {
                if (data.status === 'ok') {
                    if (data.data && data.data.redirect) {
                        document.location.replace('#successRegistration');
                    }
                }
            },
            admin_login_acc: function ($form, data) {
                if (data.status === 'ok') {
                    if (data.data && data.data.redirect) {
                        window.location.href = data.data.redirect;
                    }
            ...
            admin_logout_acc: function ($form, data) {
                if (data.status === 'ok') {
                    if (data.data && data.data.redirect) {
                        window.location.href = data.data.redirect;
                    }
            ...
            admin_upd_1xbet: function ($form, data) {
                if (data.status === 'ok') {
                    if (data.data && data.data.redirect) {
                        window.location.href = data.data.redirect;
                    }
            ...
            account_donate2: function ($form, data) {
                if (data.status === 'ok') {
                    if (data.data && data.data.redirect) {
                        window.location.href = data.data.redirect;
                    }
        ...
        validateByAjax: function($form, data) {
            if (data.status === 'ok') {
                if (data.message !== undefined && data.message !== null) {
                     if (data.code === 'main') {
						//triggerSuccessModal(data.message);
					 }
                }
            } else if (data.status === 'err') {
                if (data.code === 'main') {
					new Noty({type: 'warning',text: data.message,theme: 'mint'}).show();
                    grecaptcha.reset();
                } else {
